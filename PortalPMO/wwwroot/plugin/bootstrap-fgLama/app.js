...
function sampleAcquired(s) {
    if (currentFormat == Fingerprint.SampleFormat.PngImage) {
        ...
        localStorage.setItem("imageSrc", "data:image/octet-stream;base64," + Fingerprint.b64UrlTo64(samples[0]));
        
        //var dataPost = {
        //    base64: Fingerprint.b64UrlTo64(samples[0]),
        //    url_images: "http://ofadev.id/Finger/Handika/Telunjuk_Kanan.bmp"
        //}
        var postData = new FormData();
        postData.append("base64", Fingerprint.b64UrlTo64(samples[0]))
        postData.append("url_images", "http://ofadev.id/Finger/Handika/Telunjuk_Kanan.bmp")



        //LOGIC PENGECEKAN MATCHING DISINI
        $.ajax({
            type: 'GET',
            ...
            success: function (hasil) {
                if (hasil == true) {
                    $.ajax({
                        type: 'POST',
                        url: '../Utility/MatchingFinger',
                        data: { "base64": Fingerprint.b64UrlTo64(samples[0]), "url_images": "http://ofadev.id/Finger/Handika/Telunjuk_Kanan.bmp"},
                        //cache: false,
                        //dataType: 'json',
                        //processData: false,
                        success: function (hasil) {
                            if (hasil.status == "success") {
                                if (hasil.message == "Fingerprint cocok.") {
                                    NotifikasiSukses("Sukses", hasil.message);
                                }
                                else {
                                    NotifikasiError("Error", "Data finger tidak cocok !");
                                }
                            }
                            else {
                                NotifikasiError("Error", hasil.message)
                            }
                        }
                    });
                }
        ...
        if (state == document.getElementById("content-capture")) {
            ...
            //var image = document.createElement("img");
            //image.id = "image";
            var image = document.getElementById('HasilScan');
            image.src = localStorage.getItem("imageSrc");
            //vDiv.appendChild(image);
        }
...
function checkReaderCount(sucessObj, checkForRedirecting) {
    if (sucessObj.length == 0) {
        NotifikasiError("Error", "Mesin tidak terdeteksi, Silahkan cek mesin fingerprint Anda terlebih dahulu!")

        //alert("Mesin tidak terdeteksi, Silahkan cek mesin fingerprint Anda terlebih dahulu!");
...
function populatePopUpModal() {
    ...
    if (myVal != "") {
        ...
        modelWindowElement.innerHTML = "Silahkan pilih mesin fingerprint terlebih dahulu!";
    }
}
...
function disableEnable() {
    ...
    if (myVal != "") {
        ...
        showMessage("Silahkan pilih mesin fingerprint terlebih dahulu!");
        onStop();
    }
}
