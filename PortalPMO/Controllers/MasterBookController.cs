...
namespace PortalPMO.Controllers
{
    public class MasterBookController : Controller
    {
        ...
        public MasterBookController(IConfiguration config, dbPortalPMOContext context, IHttpContextAccessor accessor)
        {
            _context = context;
        ...
        public async Task<IActionResult> LoadData()
        {
            ...
            try
            {
                ...
                list = StoredProcedureExecutor.ExecuteSPList<MasterBook_ViewModels>(_context, "[sp_MasterBook_View]", new SqlParameter[]{
                ...
                recordsTotal = StoredProcedureExecutor.ExecuteScalarInt(_context, "[sp_MasterBook_Count]", new SqlParameter[]{
                        new SqlParameter("@Kode", KodeSearchParam),
                        new SqlParameter("@NamA", NamaSearchParam)});
        ...
        public async Task<ActionResult> SubmitCreate(TblMasterBook model, IFormFile file)
        {
            ...
            if (file != null)
            {
                BaseDirectory = System.IO.Directory.GetCurrentDirectory() + "\\" + "FileRepository";
                FileName = Path.GetFileName(file.FileName);
                FileExt = Path.GetExtension(FileName);
            }
            else 
            {
                return Content("File Gambar Tidak Boleh Kosong");            
            }
        ...
        [ValidateAntiForgeryToken]
        [HttpPost]
        public async Task<ActionResult> SubmitEdit(TblMasterBook model, IFormFile file)
        {
            string BaseDirectory;
            string FileName;
            string FileExt;
            ...
            try
            {
                ...
                using (var ms = new MemoryStream())
                {
                    file.CopyTo(ms);
                    var fileBytes = ms.ToArray();
                    data.Picture = "data:image/png;base64, " + Convert.ToBase64String(fileBytes);

                    // act on the Base64 data
                }
                data.RealeaseDate = model.RealeaseDate;
