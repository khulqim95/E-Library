...
{
    public class MasterStatusProjectController : Controller
    {
        ...
        public MasterStatusProjectController(IConfiguration config, dbPortalPMOContext context, IHttpContextAccessor accessor)
        {
            _context = context;
        ...
        public IActionResult LoadData()
        {
            ...
            try
            {
                ...
                List<DataMasterStatusProject_ViewModels> list = new List<DataMasterStatusProject_ViewModels>();

                list = StoredProcedureExecutor.ExecuteSPList<DataMasterStatusProject_ViewModels>(_context, "[sp_MasterStatusProject_View]", new SqlParameter[]{
                ...
                recordsTotal = StoredProcedureExecutor.ExecuteScalarInt(_context, "[sp_MasterStatusProject_Count]", new SqlParameter[]{
                        new SqlParameter("@Kode", KodeSearchParam),
                        new SqlParameter("@NamA", NamaSearchParam)});

                if (list == null)
                {
                    list = new List<DataMasterStatusProject_ViewModels>();
                    recordsTotal = 0;
                }
        ...
        [HttpPost]
        public ActionResult SubmitCreate(TblMasterStatusProject model)
        {
            if (!lastSession.Update())
            {
                return RedirectToAction("Login", "Login", new { a = true });
            ...
            try
            {
                ...
                using (TransactionScope trx = new TransactionScope())
                {
                    ...
                    _context.TblMasterStatusProject.Add(model);
        ...
        public ActionResult Edit(int id)
        {
            ...
            TblMasterStatusProject data = _context.TblMasterStatusProject.Where(m => m.Id == id).FirstOrDefault();
            if (data == null)
            {
                data = new TblMasterStatusProject();
            }
        ...
        [ValidateAntiForgeryToken]
        [HttpPost]
        public ActionResult SubmitEdit(TblMasterStatusProject model)
        {
            try
            {
                if (!lastSession.Update())
                ...
                TblMasterStatusProject data = _context.TblMasterStatusProject.Where(m => m.Id == model.Id).FirstOrDefault(); // Ambil data sesuai dengan ID
        ...
        public ActionResult Delete(string Ids)
        {
            ...
            try
            {
                ...
                List<TblMasterStatusProject> Transaksis = _context.TblMasterStatusProject.Where(x => confirmedDeleteId.Contains(x.Id)).ToList(); //Ambil data sesuai dengan ID
                for (int i = 0; i < confirmedDeleteId.Length; i++)
                {
                    TblMasterStatusProject data = _context.TblMasterStatusProject.Find(Transaksis[i].Id);
