...
{
    public class MasterTypeClientController : Controller
    {
        ...
        public MasterTypeClientController(IConfiguration config, dbPortalPMOContext context, IHttpContextAccessor accessor)
        {
            _context = context;
        ...
        public IActionResult LoadData()
        {
            ...
            try
            {
                ...
                List<DataMasterTypeClient_ViewModels> list = new List<DataMasterTypeClient_ViewModels>();

                list = StoredProcedureExecutor.ExecuteSPList<DataMasterTypeClient_ViewModels>(_context, "[sp_MasterTypeClient_View]", new SqlParameter[]{
                ...
                recordsTotal = StoredProcedureExecutor.ExecuteScalarInt(_context, "[sp_MasterTypeClient_Count]", new SqlParameter[]{
                        new SqlParameter("@Kode", KodeSearchParam),
                        new SqlParameter("@NamA", NamaSearchParam)});

                if (list == null)
                {
                    list = new List<DataMasterTypeClient_ViewModels>();
                    recordsTotal = 0;
                }
        ...
        [HttpPost]
        public ActionResult SubmitCreate(TblMasterTypeClient model)
        {
            if (!lastSession.Update())
            {
                return RedirectToAction("Login", "Login", new { a = true });
            ...
            try
            {
                ...
                using (TransactionScope trx = new TransactionScope())
                {
                    ...
                    _context.TblMasterTypeClient.Add(model);
        ...
        public ActionResult Edit(int id)
        {
            ...
            TblMasterTypeClient data = _context.TblMasterTypeClient.Where(m => m.Id == id).FirstOrDefault();
            if (data == null)
            {
                data = new TblMasterTypeClient();
            }
        ...
        [ValidateAntiForgeryToken]
        [HttpPost]
        public ActionResult SubmitEdit(TblMasterTypeClient model)
        {
            try
            {
                if (!lastSession.Update())
                ...
                TblMasterTypeClient data = _context.TblMasterTypeClient.Where(m => m.Id == model.Id).FirstOrDefault(); // Ambil data sesuai dengan ID
        ...
        public ActionResult Delete(string Ids)
        {
            ...
            try
            {
                ...
                List<TblMasterTypeClient> Transaksis = _context.TblMasterTypeClient.Where(x => confirmedDeleteId.Contains(x.Id)).ToList(); //Ambil data sesuai dengan ID
                for (int i = 0; i < confirmedDeleteId.Length; i++)
                {
                    TblMasterTypeClient data = _context.TblMasterTypeClient.Find(Transaksis[i].Id);
