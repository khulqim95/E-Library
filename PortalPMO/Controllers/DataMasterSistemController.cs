...
{
    public class DataMasterSistemController : Controller
    {
        ...
        public DataMasterSistemController(IConfiguration config, dbPortalPMOContext context, IHttpContextAccessor accessor)
        {
            _context = context;
        ...
        public IActionResult LoadData()
        {
            ...
            try
            {
                ...
                List<DataMasterSistem_ViewModels> list = new List<DataMasterSistem_ViewModels>();

                list = StoredProcedureExecutor.ExecuteSPList<DataMasterSistem_ViewModels>(_context, "[sp_MasterSistem_View]", new SqlParameter[]{
                ...
                recordsTotal = StoredProcedureExecutor.ExecuteScalarInt(_context, "[sp_MasterSistem_count]", new SqlParameter[]{
                        new SqlParameter("@Kode", KodeSearchParam),
                        new SqlParameter("@NamA", NamaSearchParam)});

                if (list == null)
                {
                    list = new List<DataMasterSistem_ViewModels>();
                    recordsTotal = 0;
                }
        ...
        [ValidateAntiForgeryToken]
        [HttpPost]
        public ActionResult SubmitEdit(TblMasterSistem model)
        {
            try
            {
                if (!lastSession.Update())
                ...
                TblMasterSistem data = _context.TblMasterSistem.Where(m => m.Id == model.Id).FirstOrDefault(); // Ambil data sesuai dengan ID
