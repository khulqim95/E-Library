...
namespace PortalPMO.Controllers
{
    public class MasterRoleController : Controller
    {
        ...
        public MasterRoleController(IConfiguration config, dbPortalPMOContext context, IHttpContextAccessor accessor)
        {
            _context = context;
        ...
        public IActionResult LoadData()
        {
            ...
            try
            {
                ...
                List<PengaturanMasterRole_ViewModels> list = new List<PengaturanMasterRole_ViewModels>();

                list = StoredProcedureExecutor.ExecuteSPList<PengaturanMasterRole_ViewModels>(_context, "sp_PengaturanMasterRole_View", new SqlParameter[]{
                ...
                recordsTotal = StoredProcedureExecutor.ExecuteScalarInt(_context, "[sp_PengaturanMasterRole_Count]", new SqlParameter[]{
                          new SqlParameter("@Nama", NamaSearchParam),
                        new SqlParameter("@Keterangan", KeteranganSearchParam)});

                if (list == null)
                {
                    list = new List<PengaturanMasterRole_ViewModels>();
                    recordsTotal = 0;
                }
        ...
        [HttpPost]
        public ActionResult SubmitCreate(TblMasterRole model)
        {
            if (!lastSession.Update())
            {
                return RedirectToAction("Login", "Login", new { a = true });
            ...
            try
            {
                ...
                using (TransactionScope trx = new TransactionScope())
                {
                    ...
                    _context.TblMasterRole.Add(model);
        ...
        public ActionResult Edit(int id)
        {
            ...
            TblMasterRole data = _context.TblMasterRole.Where(m => m.Id == id).FirstOrDefault();
            if (data == null)
            {
        ...
        public ActionResult Delete(string Ids)
        {
            ...
            try
            {
                ...
                List<TblMasterRole> Transaksis = _context.TblMasterRole.Where(x => confirmedDeleteId.Contains(x.Id)).ToList(); //Ambil data sesuai dengan ID
                for (int i = 0; i < confirmedDeleteId.Length; i++)
                {
                    TblMasterRole data = _context.TblMasterRole.Find(Transaksis[i].Id);
