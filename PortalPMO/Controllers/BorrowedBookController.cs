...
namespace PortalPMO.Controllers
{
    public class BorrowedBookController : Controller
    {
        ...
        public BorrowedBookController(IConfiguration config, dbPortalPMOContext context, IHttpContextAccessor accessor)
        {
            _context = context;
        ...
        public async Task<IActionResult> LoadData()
        {
            ...
            try
            {
                ...
                int recordsTotal = 0;
                var userId = HttpContext.Session.GetString(SessionConstan.Session_Pegawai_Id);

                List <MasterBook_ViewModels> list = new List<MasterBook_ViewModels>();

                list = StoredProcedureExecutor.ExecuteSPList<MasterBook_ViewModels>(_context, "[sp_BorrowedBook_View]", new SqlParameter[]{
                ...
                recordsTotal = StoredProcedureExecutor.ExecuteScalarInt(_context, "[sp_BorrowedBook_Count]", new SqlParameter[]{
                        new SqlParameter("@Kode", KodeSearchParam),
                        new SqlParameter("@UserId", userId),
                        new SqlParameter("@NamA", NamaSearchParam)});
        ...
        [ValidateAntiForgeryToken]
        [HttpPost]
        public async Task<ActionResult> SubmitReturn(TblMasterBook model, DateTime BorrowDate, DateTime FinishDate)
        {
            try
            {
                if (!lastSession.Update())
                ...
                TblBorrowedBook databook = _context.TblBorrowedBook.Where(m => m.IdBook == model.Id && m.IdUser == userId).FirstOrDefault(); // Ambil data sesuai dengan ID    

                databook.IsLate = false;

                TblMasterBook data = _context.TblMasterBook.Where(m => m.Id == model.Id).FirstOrDefault(); // Ambil data sesuai dengan ID                
