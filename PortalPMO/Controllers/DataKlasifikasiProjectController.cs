...
{
    public class DataKlasifikasiProjectController : Controller
    {
        ...
        public DataKlasifikasiProjectController(IConfiguration config, dbPortalPMOContext context, IHttpContextAccessor accessor)
        {
            _context = context;
        ...
        public IActionResult LoadData()
        {
            ...
            try
            {
                ...
                List<DataKlasifikasiProject_ViewModels> list = new List<DataKlasifikasiProject_ViewModels>();

                list = StoredProcedureExecutor.ExecuteSPList<DataKlasifikasiProject_ViewModels>(_context, "[sp_MasterKlasifikasiProject_View]", new SqlParameter[]{
                ...
                recordsTotal = StoredProcedureExecutor.ExecuteScalarInt(_context, "[sp_MasterKlasifikasiProject_Count]", new SqlParameter[]{
                        new SqlParameter("@Kode", KodeSearchParam),
                        new SqlParameter("@NamA", NamaSearchParam)});

                if (list == null)
                {
                    list = new List<DataKlasifikasiProject_ViewModels>();
                    recordsTotal = 0;
                }
        ...
        [HttpPost]
        public ActionResult SubmitCreate(TblMasterKlasifikasiProject model)
        {
            if (!lastSession.Update())
            {
                return RedirectToAction("Login", "Login", new { a = true });
            ...
            try
            {
                ...
                using (TransactionScope trx = new TransactionScope())
                {
                    ...
                    _context.TblMasterKlasifikasiProject.Add(model);
        ...
        public ActionResult Edit(int id)
        {
            ...
            TblMasterKlasifikasiProject data = _context.TblMasterKlasifikasiProject.Where(m => m.Id == id).FirstOrDefault();
            if (data == null)
            {
                data = new TblMasterKlasifikasiProject();
            }
        ...
        [ValidateAntiForgeryToken]
        [HttpPost]
        public ActionResult SubmitEdit(TblMasterKlasifikasiProject model)
        {
            try
            {
                if (!lastSession.Update())
                ...
                TblMasterKlasifikasiProject data = _context.TblMasterKlasifikasiProject.Where(m => m.Id == model.Id).FirstOrDefault(); // Ambil data sesuai dengan ID
        ...
        public ActionResult Delete(string Ids)
        {
            ...
            try
            {
                ...
                List<TblMasterKlasifikasiProject> Transaksis = _context.TblMasterKlasifikasiProject.Where(x => confirmedDeleteId.Contains(x.Id)).ToList(); //Ambil data sesuai dengan ID
                for (int i = 0; i < confirmedDeleteId.Length; i++)
                {
                    TblMasterKlasifikasiProject data = _context.TblMasterKlasifikasiProject.Find(Transaksis[i].Id);
