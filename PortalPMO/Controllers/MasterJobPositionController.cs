...
{
    public class MasterJobPositionController : Controller
    {
        ...
        public MasterJobPositionController(IConfiguration config, dbPortalPMOContext context, IHttpContextAccessor accessor)
        {
            _context = context;
        ...
        public IActionResult LoadData()
        {
            ...
            try
            {
                ...
                List<DataJobPosition_ViewModels> list = new List<DataJobPosition_ViewModels>();

                list = StoredProcedureExecutor.ExecuteSPList<DataJobPosition_ViewModels>(_context, "[sp_MasterJobPosition_View]", new SqlParameter[]{
                ...
                recordsTotal = StoredProcedureExecutor.ExecuteScalarInt(_context, "[sp_MasterJobPosition_Count]", new SqlParameter[]{
                        new SqlParameter("@Kode", KodeSearchParam),
                        new SqlParameter("@NamA", NamaSearchParam)});

                if (list == null)
                {
                    list = new List<DataJobPosition_ViewModels>();
                    recordsTotal = 0;
                }
        ...
        [HttpPost]
        public ActionResult SubmitCreate(TblMasterJobPosition model)
        {
            if (!lastSession.Update())
            {
                return RedirectToAction("Login", "Login", new { a = true });
            ...
            try
            {
                ...
                using (TransactionScope trx = new TransactionScope())
                {
                    ...
                    _context.TblMasterJobPosition.Add(model);
        ...
        public ActionResult Edit(int id)
        {
            ...
            TblMasterJobPosition data = _context.TblMasterJobPosition.Where(m => m.Id == id).FirstOrDefault();
            if (data == null)
            {
                data = new TblMasterJobPosition();
            }
        ...
        [ValidateAntiForgeryToken]
        [HttpPost]
        public ActionResult SubmitEdit(TblMasterJobPosition model)
        {
            try
            {
                if (!lastSession.Update())
                ...
                TblMasterJobPosition data = _context.TblMasterJobPosition.Where(m => m.Id == model.Id).FirstOrDefault(); // Ambil data sesuai dengan ID
        ...
        public ActionResult Delete(string Ids)
        {
            ...
            try
            {
                ...
                List<TblMasterJobPosition> Transaksis = _context.TblMasterJobPosition.Where(x => confirmedDeleteId.Contains(x.Id)).ToList(); //Ambil data sesuai dengan ID
                for (int i = 0; i < confirmedDeleteId.Length; i++)
                {
                    TblMasterJobPosition data = _context.TblMasterJobPosition.Find(Transaksis[i].Id);
