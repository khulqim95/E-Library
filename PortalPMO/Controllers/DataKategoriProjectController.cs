...
{
    public class DataKategoriProjectController : Controller
    {
        ...
        public DataKategoriProjectController(IConfiguration config, dbPortalPMOContext context, IHttpContextAccessor accessor)
        {
            _context = context;
        ...
        public IActionResult LoadData()
        {
            ...
            try
            {
                ...
                List<DataKategoriProject_ViewModels> list = new List<DataKategoriProject_ViewModels>();

                list = StoredProcedureExecutor.ExecuteSPList<DataKategoriProject_ViewModels>(_context, "[sp_MasterKategoriProject_View]", new SqlParameter[]{
                ...
                recordsTotal = StoredProcedureExecutor.ExecuteScalarInt(_context, "[sp_MasterKategoriProject_Count]", new SqlParameter[]{
                        new SqlParameter("@Kode", KodeSearchParam),
                        new SqlParameter("@NamA", NamaSearchParam)});

                if (list == null)
                {
                    list = new List<DataKategoriProject_ViewModels>();
                    recordsTotal = 0;
                }
        ...
        [HttpPost]
        public ActionResult SubmitCreate(TblMasterKategoriProject model)
        {
            if (!lastSession.Update())
            {
                return RedirectToAction("Login", "Login", new { a = true });
            ...
            try
            {
                ...
                using (TransactionScope trx = new TransactionScope())
                {
                    ...
                    _context.TblMasterKategoriProject.Add(model);
        ...
        public ActionResult Edit(int id)
        {
            ...
            TblMasterKategoriProject data = _context.TblMasterKategoriProject.Where(m => m.Id == id).FirstOrDefault();
            if (data == null)
            {
                data = new TblMasterKategoriProject();
            }
        ...
        [ValidateAntiForgeryToken]
        [HttpPost]
        public ActionResult SubmitEdit(TblMasterKategoriProject model)
        {
            try
            {
                if (!lastSession.Update())
                ...
                TblMasterKategoriProject data = _context.TblMasterKategoriProject.Where(m => m.Id == model.Id).FirstOrDefault(); // Ambil data sesuai dengan ID
        ...
        public ActionResult Delete(string Ids)
        {
            ...
            try
            {
                ...
                List<TblMasterKategoriProject> Transaksis = _context.TblMasterKategoriProject.Where(x => confirmedDeleteId.Contains(x.Id)).ToList(); //Ambil data sesuai dengan ID
                for (int i = 0; i < confirmedDeleteId.Length; i++)
                {
                    TblMasterKategoriProject data = _context.TblMasterKategoriProject.Find(Transaksis[i].Id);
