...
{
    public class MasterPertanyaanController : Controller
    {
        ...
        public MasterPertanyaanController(IConfiguration config, dbPipelineContext context, IHttpContextAccessor accessor)
        {
            _context = context;
        ...
        public IActionResult LoadData()
        {
            ...
            try
            {
                ...
                List<MasterPertanyaan_ViewModels> list = new List<MasterPertanyaan_ViewModels>();

                list = StoredProcedureExecutor.ExecuteSPList<MasterPertanyaan_ViewModels>(_context, "sp_MasterPertanyaan_View", new SqlParameter[]{
                        new SqlParameter("@Pertanyaan", NamaSearchParam),
                        new SqlParameter("@sortColumn", sortColumn),
                        new SqlParameter("@sortColumnDir", sortColumnDir),
                        new SqlParameter("@PageNumber", pageNumber),
                        new SqlParameter("@RowsPage", pageSize)});

                recordsTotal = StoredProcedureExecutor.ExecuteScalarInt(_context, "[sp_MasterPertanyaan_Count]", new SqlParameter[]{
                          new SqlParameter("@Pertanyaan", NamaSearchParam)
                });

                if (list == null)
                {
                    list = new List<MasterPertanyaan_ViewModels>();
                    recordsTotal = 0;
                }
        ...
        public ActionResult Create()
        {
            ...
            ViewBag.TypeSoal = new SelectList(UtilityPipeline.SelectLookup("TypeSoal", _context), "Value", "Name");
        ...
        [HttpPost]
        public ActionResult SubmitCreate(TblMasterPertanyaan model)
        {
            if (!lastSession.Update())
            {
                return RedirectToAction("Login", "Login", new { a = true });
            ...
            try
            {
                ...
                using (TransactionScope trx = new TransactionScope())
                {
                    ...
                    model.DefaultJawaban = false;
                    model.IsMandatory = true;
                    _context.TblMasterPertanyaan.Add(model);
        ...
        public ActionResult Edit(int id)
        {
            ...
            TblMasterPertanyaan data = _context.TblMasterPertanyaan.Where(m => m.Id == id).FirstOrDefault();
            ViewBag.TypeSoal = new SelectList(UtilityPipeline.SelectLookup("TypeSoal", _context), "Value", "Name", data.Type);
            if (data == null)
            {
                data = new TblMasterPertanyaan();
            }
        ...
        [ValidateAntiForgeryToken]
        [HttpPost]
        public ActionResult SubmitEdit(TblMasterRole model)
        {
            try
            {
                if (!lastSession.Update())
                ...
                TblMasterRole data = _context.TblMasterRole.Where(m => m.Id == model.Id).FirstOrDefault(); // Ambil data sesuai dengan ID
        ...
        //        List<TblMasterPertanyaan> Transaksis = _context.TblMasterPertanyaan.Where(x => confirmedDeleteId.Contains(x.Id)).ToList(); //Ambil data sesuai dengan ID
        //        for (int i = 0; i < confirmedDeleteId.Length; i++)
        //        {
        //            TblMasterPertanyaan data = _context.TblMasterPertanyaan.Find(Transaksis[i].Id);
