...
{
    public class DataSubKategoriProjectController : Controller
    {
        ...
        public DataSubKategoriProjectController(IConfiguration config, dbPortalPMOContext context, IHttpContextAccessor accessor)
        {
            _context = context;
        ...
        public IActionResult LoadData()
        {
            ...
            try
            {
                ...
                List<DataSubKategoriProject_ViewModels> list = new List<DataSubKategoriProject_ViewModels>();

                list = StoredProcedureExecutor.ExecuteSPList<DataSubKategoriProject_ViewModels>(_context, "[sp_MasterSubKategoriProject_View]", new SqlParameter[]{
                ...
                recordsTotal = StoredProcedureExecutor.ExecuteScalarInt(_context, "[sp_MasterSubKategoriProject_Count]", new SqlParameter[]{
                        new SqlParameter("@Kode", KodeSearchParam),
                        new SqlParameter("@NamA", NamaSearchParam)});

                if (list == null)
                {
                    list = new List<DataSubKategoriProject_ViewModels>();
                    recordsTotal = 0;
                }
        ...
        [HttpPost]
        public ActionResult SubmitCreate(TblMasterSubKategoriProject model)
        {
            if (!lastSession.Update())
            {
                return RedirectToAction("Login", "Login", new { a = true });
            ...
            try
            {
                ...
                using (TransactionScope trx = new TransactionScope())
                {
                    ...
                    _context.TblMasterSubKategoriProject.Add(model);
        ...
        public ActionResult Edit(int id)
        {
            ...
            TblMasterSubKategoriProject data = _context.TblMasterSubKategoriProject.Where(m => m.Id == id).FirstOrDefault();
            if (data == null)
            {
                data = new TblMasterSubKategoriProject();
            }
        ...
        [ValidateAntiForgeryToken]
        [HttpPost]
        public ActionResult SubmitEdit(TblMasterSubKategoriProject model)
        {
            try
            {
                if (!lastSession.Update())
                ...
                TblMasterSubKategoriProject data = _context.TblMasterSubKategoriProject.Where(m => m.Id == model.Id).FirstOrDefault(); // Ambil data sesuai dengan ID
        ...
        public ActionResult Delete(string Ids)
        {
            ...
            try
            {
                ...
                List<TblMasterSubKategoriProject> Transaksis = _context.TblMasterSubKategoriProject.Where(x => confirmedDeleteId.Contains(x.Id)).ToList(); //Ambil data sesuai dengan ID
                for (int i = 0; i < confirmedDeleteId.Length; i++)
                {
                    TblMasterSubKategoriProject data = _context.TblMasterSubKategoriProject.Find(Transaksis[i].Id);
