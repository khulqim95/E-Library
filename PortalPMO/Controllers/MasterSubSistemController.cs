...
{
    public class MasterSubSistemController : Controller
    {
        ...
        public MasterSubSistemController(IConfiguration config, dbPortalPMOContext context, IHttpContextAccessor accessor)
        {
            _context = context;
        ...
        public IActionResult LoadData()
        {
            ...
            try
            {
                ...
                var KodeSistemSearchParam = dict["columns[1][search][value]"];
                var NamaSistemSearchParam = dict["columns[2][search][value]"];
                var KodeSearchParam = dict["columns[3][search][value]"];
                var NamaSearchParam = dict["columns[4][search][value]"];
                ...
                List<DataMasterSubSistem_ViewModels> list = new List<DataMasterSubSistem_ViewModels>();

                list = StoredProcedureExecutor.ExecuteSPList<DataMasterSubSistem_ViewModels>(_context, "[sp_MasterSubSistem_View]", new SqlParameter[]{
                        new SqlParameter("@Kode", KodeSearchParam),
                        new SqlParameter("@Nama", NamaSearchParam),
                        new SqlParameter("@KodeSistem", KodeSistemSearchParam),
                        new SqlParameter("@NamaSistem", NamaSistemSearchParam),
                        new SqlParameter("@sortColumn", sortColumn),
                        new SqlParameter("@sortColumnDir", sortColumnDir),
                        new SqlParameter("@PageNumber", pageNumber),
                        new SqlParameter("@RowsPage", pageSize)});

                recordsTotal = StoredProcedureExecutor.ExecuteScalarInt(_context, "[sp_MasterSubSistem_Count]", new SqlParameter[]{
                        new SqlParameter("@Kode", KodeSearchParam),
                        new SqlParameter("@Nama", NamaSearchParam),
                        new SqlParameter("@KodeSistem", KodeSistemSearchParam),
                        new SqlParameter("@NamaSistem", NamaSistemSearchParam)});
                ...
                if (list == null)
                {
                    list = new List<DataMasterSubSistem_ViewModels>();
                    recordsTotal = 0;
                }
        ...
        [HttpPost]
        public ActionResult SubmitCreate(TblMasterSubSistem model)
        {
            if (!lastSession.Update())
            {
                return RedirectToAction("Login", "Login", new { a = true });
            ...
            try
            {
                ...
                using (TransactionScope trx = new TransactionScope())
                {
                    ...
                    _context.TblMasterSubSistem.Add(model);
        ...
        public ActionResult Edit(int id)
        {
            ...
            TblMasterSubSistem data = _context.TblMasterSubSistem.Where(m => m.Id == id).FirstOrDefault();
            if (data == null)
            {
                data = new TblMasterSubSistem();
            }

            if (data.MasterSistemId != null)
            {
                ViewBag.SistemId = new SelectList(Utility.SelectDataMasterSistem(data.MasterSistemId, _context).ToList(), "id", "text", data.MasterSistemId);
            }
            else
            {
                ViewBag.SistemId = new SelectList("", "");
            }
        ...
        [ValidateAntiForgeryToken]
        [HttpPost]
        public ActionResult SubmitEdit(TblMasterSubSistem model)
        {
            try
            {
                if (!lastSession.Update())
                ...
                TblMasterSubSistem data = _context.TblMasterSubSistem.Where(m => m.Id == model.Id).FirstOrDefault(); // Ambil data sesuai dengan ID
                data.MasterSistemId = model.MasterSistemId;
        ...
        public ActionResult Delete(string Ids)
        {
            ...
            try
            {
                ...
                List<TblMasterSubSistem> Transaksis = _context.TblMasterSubSistem.Where(x => confirmedDeleteId.Contains(x.Id)).ToList(); //Ambil data sesuai dengan ID
                for (int i = 0; i < confirmedDeleteId.Length; i++)
                {
                    TblMasterSubSistem data = _context.TblMasterSubSistem.Find(Transaksis[i].Id);
