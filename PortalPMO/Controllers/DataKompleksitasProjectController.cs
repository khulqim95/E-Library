...
{
    public class DataKompleksitasProjectController : Controller
    {
        ...
        public DataKompleksitasProjectController(IConfiguration config, dbPortalPMOContext context, IHttpContextAccessor accessor)
        {
            _context = context;
        ...
        public IActionResult LoadData()
        {
            ...
            try
            {
                ...
                List<DataKompleksitasProject_ViewModels> list = new List<DataKompleksitasProject_ViewModels>();

                list = StoredProcedureExecutor.ExecuteSPList<DataKompleksitasProject_ViewModels>(_context, "[sp_MasterKompleksitasProject_View]", new SqlParameter[]{
                ...
                recordsTotal = StoredProcedureExecutor.ExecuteScalarInt(_context, "[sp_MasterKompleksitasProject_Count]", new SqlParameter[]{
                        new SqlParameter("@Kode", KodeSearchParam),
                        new SqlParameter("@NamA", NamaSearchParam)});

                if (list == null)
                {
                    list = new List<DataKompleksitasProject_ViewModels>();
                    recordsTotal = 0;
                }
        ...
        public ActionResult Create()
        {
            ...
            ViewBag.KategoriKompleksitas = new SelectList(Utility.SelectLookup("KategoriKompleksitas", _context), "Value", "Name");
        ...
        [HttpPost]
        public ActionResult SubmitCreate(TblMasterKompleksitasProject model)
        {
            if (!lastSession.Update())
            {
                return RedirectToAction("Login", "Login", new { a = true });
            ...
            try
            {
                ...
                using (TransactionScope trx = new TransactionScope())
                {
                    ...
                    _context.TblMasterKompleksitasProject.Add(model);
        ...
        public ActionResult Edit(int id)
        {
            ...
            TblMasterKompleksitasProject data = _context.TblMasterKompleksitasProject.Where(m => m.Id == id).FirstOrDefault();
            if (data == null)
            {
                data = new TblMasterKompleksitasProject();
            }

            ViewBag.KategoriKompleksitas = new SelectList(Utility.SelectLookup("KategoriKompleksitas", _context).ToList(), "Value", "Name", data.KategoriKompleksitasId);
        ...
        [ValidateAntiForgeryToken]
        [HttpPost]
        public ActionResult SubmitEdit(TblMasterKompleksitasProject model)
        {
            try
            {
                if (!lastSession.Update())
                ...
                TblMasterKompleksitasProject data = _context.TblMasterKompleksitasProject.Where(m => m.Id == model.Id).FirstOrDefault(); // Ambil data sesuai dengan ID
                data.Kode = model.Kode;
                data.Nama = model.Nama;
                data.KategoriKompleksitasId = model.KategoriKompleksitasId;
        ...
        public ActionResult Delete(string Ids)
        {
            ...
            try
            {
                ...
                List<TblMasterKompleksitasProject> Transaksis = _context.TblMasterKompleksitasProject.Where(x => confirmedDeleteId.Contains(x.Id)).ToList(); //Ambil data sesuai dengan ID
                for (int i = 0; i < confirmedDeleteId.Length; i++)
                {
                    TblMasterKompleksitasProject data = _context.TblMasterKompleksitasProject.Find(Transaksis[i].Id);
